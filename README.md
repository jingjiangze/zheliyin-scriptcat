...
| **Demo（当前分支）** | `demo` | **`0.3.6.0`** | 真实用户试用 AI-1 最新实现 |
| 开发中 | `stage-4.1-runtime-validation` | — | AI-1 持续开发，不建议直接使用 |

> 下方标题里的 `v0.2.3.11` 是旧版遗留文案；**本 Demo 的实际版本以上表 `0.3.6.0` 为准**（面板内「版本：」亦显示该值）。

**Demo 已知情况（截至 Stage 5.5B）**：图片识别支持「本地 Tesseract.js」与「百度云端 OCR」双通道（自动模式本地优先、失败切换云端）；背景图/选中图均可识别。套版填层等既有功能正常。
