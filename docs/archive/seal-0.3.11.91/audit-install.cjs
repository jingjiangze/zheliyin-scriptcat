// 安装 audit 真机 runner + 版本 bump 0.3.11.88 -> 0.3.11.89
"use strict";
const fs = require("fs");
const path = require("path");
const ROOT = "D:/zheliyin-scriptcat";
const DST = path.join(ROOT, "runtime", "stage9", "commit-46x-native-truth-resilience-real.js");
const content = fs.readFileSync(path.join(__dirname, "audit-runner.txt"), "utf8").split("\r\n").join("\n");
fs.writeFileSync(DST, content, "utf8");
console.log("wrote " + path.relative(ROOT, DST).split(path.sep).join("/") + " (" + content.length + " bytes)");

const OLD = "0.3.11.88", NEW = "0.3.11.89";
