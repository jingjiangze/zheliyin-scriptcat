// P3 诊断：多版模式下注入槽位后，直接读画布对象 vs getTextInventoryAll，定位读取空的原因
"use strict";
...
const AI_KEY = process.env.ZY_AI_KEY || "x";
...
(async () => {
  ...
  }), { type: type, payload: payload, replyType: replyType, timeoutMs: timeoutMs || 10000 });
  try {
    ...
    await SLEEP(2500);
    // 开多版
    const opened = await page.evaluate(() => { const el = document.querySelector(".pageWrapMulti li .hover-tips a.copy-template"); if (!el) return false; try { el.click(); return true; } catch (e) { return false; } });
    await SLEEP(2500);
    const applied = await page.evaluate(() => {
      ...
      if (!mn || !un) return "NO_INPUT";
      set(mn, 1); set(un, 1);
      const ok = Array.from(document.querySelectorAll(".layui-layer-btn a")).find((b) => /确定/.test(String(b.textContent || "").trim()));
      if (!ok) return "NO_OK"; ok.click(); return "OK";
    });
    await SLEEP(12000);
    console.log("[diag] opened=" + opened + " apply=" + applied);
    // inject userscript
    await page.evaluate(({ key }) => { const set = (k, v) => { try { localStorage.setItem("zy8dshim:" + k, JSON.stringify(v)); } catch (e) {} }; set("zyShowTemplatePanel", "1"); set("zyArkApiKey", key); }, { key: AI_KEY });
    ...
    const inj = await page.evaluate(() => {
      ...
      const defs = [{ text: "启诚科技有限公司", fontSize: 16 }, { text: "王晓明", fontSize: 22 }];
      ...
      defs.forEach(function (s, i) {
        try {
          ...
          d2.drawText(s.text, null, null, null, entry, baseLayer2 + i); made += 1;
        } catch (e) { errs.push(String(i) + ":" + String(e && e.message || e).slice(0, 80)); }
      });
      try { d2.canvas.requestRenderAll(); } catch (e) {}
      return { ok: made === defs.length, made: made, errs: errs };
    });
    await SLEEP(1500);
    // 直接读画布对象
    const direct = await page.evaluate(() => {
      ...
      const out = [];
      if (Array.isArray(total)) total.forEach((e, i) => { let n = null, t = 0, err = null; try { const c = e && e.canvas; if (c && c.getObjects) { const objs = c.getObjects(); n = objs.length; t = objs.filter((o) => o && (o.type === "text" || o.type === "textbox" || o.type === "i-text" || typeof o.getText === "function")).length; } } catch (ex) { err = String(ex && ex.message || ex); } out.push({ idx: i, hasCanvas: !!(e && e.canvas), objs: n, textObjs: t, err: err }); });
      return { totalLen: Array.isArray(total) ? total.length : null, currentCanvasNum: CV && CV.currentCanvasNum, out: out };
    });
    const invAll = await bridgeCall("getTextInventoryAll", {}, "getTextInventoryAllResult", 10000);
    const invV0 = await bridgeCall("getTextInventoryAll", { version: 0 }, "getTextInventoryAllResult", 10000);
    const mvi = await bridgeCall("getMultiVersionInfo", {}, "getMultiVersionInfoResult", 10000);
    console.log("[diag] inject=" + JSON.stringify(inj));
    console.log("[diag] direct=" + JSON.stringify(direct));
    console.log("[diag] invAll: front=" + (invAll && invAll.front && (invAll.front.items || []).length) + " back=" + (invAll && invAll.back && (invAll.back.items || []).length) + " version=" + (invAll && invAll.version) + " vc=" + (invAll && invAll.versionCount) + " exists=" + (invAll && invAll.front && invAll.front.exists));
    console.log("[diag] invV0: front=" + (invV0 && invV0.front && (invV0.front.items || []).length) + " back=" + (invV0 && invV0.back && (invV0.back.items || []).length) + " exists=" + (invV0 && invV0.front && invV0.front.exists));
    console.log("[diag] mvi.versions=" + JSON.stringify(mvi && mvi.versions));
    console.log("[diag] mvi.current=" + JSON.stringify(mvi && mvi.current));
  } catch (e) { console.error("[diag] ERR " + String(e && e.message || e)); }
