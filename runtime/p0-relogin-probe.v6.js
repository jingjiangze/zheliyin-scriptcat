// runtime/p0-relogin-probe.v6.js — 交稿弹层搜索按钮精确定位 + 连点搜索 + 生成中处理
"use strict";
const path = require("path");
const fs = require("fs");
const { chromium } = require("playwright");
const EDITOR_URL = "https://diy.zheliyin.com/diyWeb/third/252438/2114747/999/thirdDiyAdd.do";
const REPORT_PATH = path.join(__dirname, "reports", "p0-relogin-v6.json");

(async () => {
  const report = { ts: new Date().toISOString(), stage: "P0-RELOGIN-V6", url: EDITOR_URL, phases: {}, resp: [], events: [], errors: [] };
  ...
  try {
    ...
    page.on("response", async (r) => {
      try { const u = r.url(); if (u.indexOf("zheliyin.com") < 0) return; const ct = r.headers()["content-type"] || ""; if (ct.indexOf("json") < 0) return; let b = ""; try { b = await r.text().catch(() => ""); } catch (e) {} if (report.resp.length < 120) report.resp.push({ status: r.status(), u: u.slice(0, 200), body: String(b).slice(0, 2200) }); } catch (e) {}
    });
    ...
    await ev(() => {
      ...
      for (let i = 0; i < vo.totalCanvasArray.length; i++) {
        ...
        const entry = { media: { mediaType: "text", text: "P0_V6", font: { pointSize: 24, fontColor: "#000000", isHorizontal: 1, gravity: "left", id: "556", isItalic: 0, textDecoration: "", linethrough: 0, overline: 0, isBold: 0, overprintStroke: 0 }, charSpace: 0, lineSpace: 1.3, lineIdType: 0, isBG: 0, imgPath: "" }, location: { x: 100, y: 100, width: 220, height: 40, factWidth: 220, factHeight: 40, rotation: 0 }, printLocation: { x: 100, y: 100, width: 220, height: 40, rotation: 0 }, layer: { alpha: 1 }, layerNum: base, isEdit: 1, isDisplay: 1, deleteState: 0, visitLevel: 1, multiUuid: "p0v6" + Date.now() % 10000, markuuid: "", topEnable: 1, resourceType: 0, maskEnable: 0, lowPixelFlag: 0, selectEnabled: 1, isDesign: 1, isComposite: 0, isPreview: 0, isDesignShape: 0 };
        d.drawText("P0_V6", null, null, null, entry, base);
    ...
    // 等「提交稿件（交稿）」弹层出现，dump 其完整结构
    let jg = null;
    const tD = Date.now() + 45000;
    while (Date.now() < tD) {
      jg = await ev(() => {
        const mods = document.querySelectorAll(".modal, .layui-layer");
        for (let i = 0; i < mods.length; i++) {
          const el = mods[i];
          if (el.offsetParent && /提交稿件|顾客信息|错字检查/.test(String(el.innerText || ""))) {
            const inputs = [];
            el.querySelectorAll("input").forEach((inp) => inputs.push({ id: inp.id, cls: String(inp.className).slice(0, 40), ph: inp.placeholder, t: String(inp.value || "").slice(0, 20) }));
            const btns = [];
            el.querySelectorAll("a, button, input[type=button], input[type=submit], span, i, em").forEach((b) => {
              if (!b.offsetParent) return;
              const tx = String(b.textContent || b.value || "").trim().slice(0, 16);
              const cls = String(b.className || "").slice(0, 50);
              if (tx || cls) btns.push({ tag: b.tagName, tx, cls, html: b.outerHTML.slice(0, 130) });
            });
            return { cls: String(el.className).slice(0, 60), inputs: inputs.slice(0, 20), btns: btns.slice(0, 60), body: el.innerText.slice(0, 900) };
          }
        }
        return null;
      });
    ...
    report.phases.jiaogaoDump = jg;

    // 连点搜索（在交稿弹层内，宽泛匹配 图标/文字 搜索|查询|icon-search + 输入框order旁按钮）
    const searchRounds = [];
    for (let r = 0; r < 4; r++) {
      const sc = await ev(() => {
        const mods = document.querySelectorAll(".modal, .layui-layer");
        let btn = null;
        // 全页面兜底（但排除顶部工具栏）
        const all = document.querySelectorAll("a, button, span, i, em, input[type=button], input[type=submit]");
        let bestMatch = null;
        for (let j = 0; j < all.length; j++) {
          ...
          const inLayer = el.closest(".modal, .layui-layer");
          // 优先弹层内：文本 搜索/查询 或 icon-search 类
          if (inLayer && (/搜索|查询|搜\s*索|查\s*询/.test(tx) || /icon-search|fa-search|search-?icon/i.test(elClass))) { btn = el; break; }
          if (!bestMatch && !/temp-info|hdgy|topBar|menuCont/.test(elClass) && (tx === "搜索" || /icon-search|fa-search|search-?icon/i.test(elClass))) bestMatch = el;
        }
        if (!btn) btn = bestMatch;
        if (!btn) {
          // 兜底：订单号输入框parent内的按钮
          const inp = Array.from(document.querySelectorAll("input")).find((el) => /order|search/i.test(String(el.className) + String(el.id)) && el.offsetParent);
          if (inp) { const wrap = inp.parentElement || inp.parentElement.parentElement; if (wrap) { const cs = wrap.querySelectorAll("a, button, span, i"); for (let k = 0; k < cs.length; k++) { if (cs[k].offsetParent) { btn = cs[k]; break; } } } }
        }
        if (!btn) return { clicked: false };
        try { btn.click(); return { clicked: true, tag: btn.tagName, cls: String(btn.className || "").slice(0, 50), tx: String(btn.textContent || "").trim().slice(0, 12) }; } catch (e) { return { clicked: false, err: String(e) }; }
      });
      searchRounds.push({ r, ...sc });
      await page.waitForTimeout(2500);
      // 处理生成中弹层
      const gen = await ev(() => {
        const mods = document.querySelectorAll(".modal, .layui-layer");
        for (let i = 0; i < mods.length; i++) { const el = mods[i]; if (el.offsetParent && /印刷稿件生成中|请耐心等待|生成中|正在生成/.test(String(el.innerText || ""))) { const btns = el.querySelectorAll("button, a"); for (let b = 0; b < btns.length; b++) { const t = String(btns[b].textContent || "").trim(); if (/确认|确定|关闭|知道了/.test(t)) { try { btns[b].click(); return { handled: true, pressed: t.slice(0, 10) }; } catch (e) {} } } } }
        return { handled: false };
      });
      if (gen && gen.handled) { report.events.push({ t: new Date().toISOString().slice(11, 19), genHandled: gen }); await page.waitForTimeout(3000); }
    }
    report.phases.searchRounds = searchRounds;

    // 观察最终核稿状态 30s
    const tF = Date.now() + 30000;
    let failLayers = null;
    while (Date.now() < tF) {
      failLayers = await ev(() => {
        const out = [];
        document.querySelectorAll(".layui-layer, .modal, [class*=hegao], [class*=check_wrap]").forEach((el) => {
          const rc = el.getBoundingClientRect(); if (rc.width === 0 && rc.height === 0) return;
          const t = String(el.innerText || el.textContent || "").trim();
          if (t && /自动核稿|核稿失败|生产文件与设计稿|标记正常|错字检查|生成完成|核稿完成|通过/.test(t)) out.push({ cls: String(el.className).slice(0, 50), txt: t.slice(0, 400) });
        });
        return out;
      });
      if (failLayers && failLayers.some((l) => /自动核稿失败|生产文件与设计稿/.test(l.txt))) break;
      await page.waitForTimeout(1500);
    }
    report.phases.failLayers = failLayers;
    report.phases.saveResp = (report.resp || []).filter((x) => /save|submit/i.test(x.u)).map((x) => ({ u: x.u.slice(0, 130), b: String(x.body).slice(0, 280) }));
  } catch (e) {
