// runtime/p0-relogin-probe.v2.js — 两阶段：触发登录浮层→自动登录→印刷全链路(保存成功验证)
"use strict";
...
const fillAndSubmit = () => {};
(async () => {
  const report = { ts: new Date().toISOString(), stage: "P0-RELOGIN-V2", url: EDITOR_URL, phases: {}, resp: [], errors: [] };
  ...
  try {
    ...
    page.on("response", async (r) => {
      try { const u = r.url(); if (u.indexOf("zheliyin.com") < 0) return; const ct = r.headers()["content-type"] || ""; if (ct.indexOf("json") < 0 && ct.indexOf("text") < 0) return; let b = ""; try { b = await r.text().catch(() => ""); } catch (e) {} if (report.resp.length < 80) report.resp.push({ status: r.status(), u: u.slice(0, 220), body: String(b).slice(0, 2500) }); } catch (e) {}
    });
    page.on("pageerror", (e) => report.errors.push(String(e && e.message || e).slice(0, 240)));
    ...
    const waitReady = async (ms) => {
      const dd = Date.now() + ms;
      let r = null;
      while (Date.now() < dd) { r = await readyEval(); if (r && r.ok) return true; await page.waitForTimeout(2000); }
      return false;
    };
    ...
    const fillForm = () => page.evaluate(() => {
      ...
      setByLabel(["备注"], "p0 relogin v2");
      return { ok: true };
    }).catch((e) => ({ err: String(e || "").slice(0, 200) }));
    const clickConfirm = () => page.evaluate(() => {
      const btns = document.querySelectorAll(".layui-layer button, .layui-layer a, .layui-layer-btn0, .modal .close ~ *, .modal button");
      let clicked = false;
      const strs = /^确定$|^保存$|^确\s*定$/;
      for (let i = 0; i < btns.length; i++) {
        const tx = String(btns[i].textContent || "").trim();
        if (strs.test(tx) && !/印刷|提交|生产|下单/.test(tx)) {
          const host = btns[i].closest(".layui-layer, .modal") || document;
          const ht = String(host.textContent || "");
          if (/确定进行印刷|提交生产|提交制作|确认提交|确定印刷|下单/i.test(ht)) continue;
          try { btns[i].click(); clicked = true; } catch (e) {}
          break;
        }
      }
      return { clicked };
    ...
    // 2) 创建对象 → 点印刷 → 填1/2 → 确定（触发 timeOut → 登录浮层）
    await page.evaluate(() => {
      const req = window.requirejs || window.require;
      const vo = ((req && req.s && req.s.contexts && req.s.contexts._ && req.s.contexts._.defined && req.s.contexts._.defined.CanvasObjVO) || window.CanvasObjVO);
    ...
    report.phases.firstConfirm = await clickConfirm();
    // 等 timeOut → 登录浮层
    let loginVisible = false;
    const d2s = Date.now() + 15000;
    while (Date.now() < d2s) {
      loginVisible = await page.evaluate(() => { const ua = document.querySelector("#userAccount"); return !!(ua && ua.offsetParent); }).catch(() => false);
    ...
    // 3) 自动登录
    if (loginVisible) {
      const lr = await page.evaluate((arg) => {
      ...
      report.phases.login = lr;
      await page.waitForTimeout(12000);
    }
    report.phases.ready2 = await waitReady(90000);
    report.phases.url = page.url().slice(0, 200);
    ...
    if (report.phases.ready2) {
      ...
      await fillForm();
      await page.waitForTimeout(1200);
      report.phases.secondConfirm = await clickConfirm();
      const t0 = Date.now();
      let dialog = null;
      while (Date.now() - t0 < 40000) {
        dialog = await page.evaluate(() => {
          ...
          document.querySelectorAll(".layui-layer, [class*=hegao]").forEach((el) => {
            ...
            if (t && /自动核稿|核稿失败|生产文件与设计稿|标记正常|去检查|保存成功|完成/.test(t)) layers.push(t.slice(0, 700));
          });
          return layers;
        }).catch(() => []);
      ...
      // 抓保存/提交响应结果
      const verdict = (report.resp || []).filter((x) => /save|submit/i.test(x.u)).map((x) => ({ u: x.u.slice(0, 120), b: String(x.body).slice(0, 260) }));
      report.phases.saveResp = verdict;
    }
  } catch (e) {
