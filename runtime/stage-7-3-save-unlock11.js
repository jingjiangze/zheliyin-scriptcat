// runtime/stage-7-3-save-unlock11.js — t4 v15：JSON.parse patch（ready 后安装）完整捕获序列化报文
// 修复 v14 bug：patch 在 goto 前执行被 reload 覆盖。本次在 ready 后 patch，另在保存前刷新 __zyParse。
// 目标：拿到含 printLocation 报文的最小成功样本（lastPL：loc/print 的 width/height）或失败样本(fails)。
"use strict";
...
const REPORT_PATH = path.join(__dirname, "reports", "stage-7-3-save-unlock11.json");

(async () => {
  const report = { ts: new Date().toISOString(), stage: "STAGE-7.3-SAVE-UNLOCK11", url: EDITOR_URL, phases: {}, dialogs: [], errors: [] };
  let browser = null;
  let page = null;
  try {
    ...
    report.phases.patchInstalled = await page.evaluate(() => {
      ...
      JSON.parse = function (s, rev) {
        ...
        if (isPL || typeof s === "string" && window.__zyParse.calls < 4) {
          try {
            const r = orig.call(JSON, s, rev);
            if (isPL) {
              window.__zyParse.calls++;
              if (r && r.location !== undefined) {
                window.__zyParse.lastPL = { len: s.length, loc: r.location, print: r.printLocation, text: r.media && r.media.text ? String(r.media.text).slice(0, 20) : null, fontId: r.media && r.media.font && r.media.font.id, mediaType: r.media && r.media.mediaType };
              }
            }
            return r;
          } catch (e) {
            if (isPL) {
              try { window.__zyParse.fails.push({ err: String(e && e.message || e).slice(0, 60), s: String(s).slice(0, 2200) }); } catch (e2) {}
            }
            throw e;
          }
        }
      ...
      return { ok: true, probe: JSON.parse('{"a":1}') && true };
    ...
    report.phases.probe = await page.evaluate(() => {
      ...
      for (let i = 0; i < vo.totalCanvasArray.length; i++) {
        ...
        const uid = "zlyunlock11-" + Date.now() % 1000000;
    ...
    const fillAndSubmit = () => page.evaluate(() => {
      ...
      setByLabel(["备注"], "stage-7.3 unlock11");
    ...
    report.phases.save1.parse = await page.evaluate(() => {
      ...
      return { fails: (z.fails || []).map((f) => ({ err: f.err, s: f.s.slice(0, 2200) })), lastPL: z.lastPL, calls: z.calls };
    }).catch((e) => ({ err: String(e || "").slice(0, 200) }));
  ...
  console.log("STAGE-7.3-SAVE-UNLOCK11 done -> " + REPORT_PATH);
  console.log("ready=" + report.phases.ready);
  console.log("patchInstalled=" + JSON.stringify(report.phases.patchInstalled));
  ...
  console.log("calls=" + p.calls + " fails=" + (p.fails || []).length);
  (p.fails || []).slice(0, 2).forEach((f, i) => { console.log("FAIL" + i + " err=" + f.err); console.log("FAIL" + i + " s=" + f.s.slice(0, 700)); });
  console.log("lastPL=" + JSON.stringify(p.lastPL));
  console.log("dialogs=" + JSON.stringify(report.dialogs));
  console.log("body=" + ((report.phases.save1 && report.phases.save1.bodySnippet) || "").slice(0, 220));
