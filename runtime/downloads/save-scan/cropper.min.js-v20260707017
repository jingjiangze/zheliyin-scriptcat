	...
	var Cropper = function () {
	  ...
	  _createClass(Cropper, [{
	    ...
	    value: function clone() {
	      ...
	      if (self.options.checkCrossOrigin && $.isCrossOriginURL(url)) {
	        ...
	        if (crossOrigin) {
	          ...
	          if(url.indexOf('img.36588.com.cn') == -1){
	    ...
	    value: function build() {
	      ...
	      //add:overall rotate)
	      if(options.pageAngle>0 && options.pageAngle<=45){
	      	cursor = ['ne','e','se','s','sw','w','nw','n'];
	      }else if(options.pageAngle>45 && options.pageAngle<=90){
	      	cursor = ['e','se','s','sw','w','nw','n','ne'];
	      }else if(options.pageAngle>90 && options.pageAngle<=135){
	      	cursor = ['se','s','sw','w','nw','n','ne','e'];
	      }else if(options.pageAngle>135 && options.pageAngle<=180){
	      	cursor = ['s','sw','w','nw','n','ne','e','se'];
	      }else if(options.pageAngle>180 && options.pageAngle<=225){
	      	cursor = ['sw','w','nw','n','ne','e','se','s'];
	      }else if(options.pageAngle>225 && options.pageAngle<=270){
	      	cursor = ['w','nw','n','ne','e','se','s','sw'];
	      }else if(options.pageAngle>270 && options.pageAngle<=315){
	      	cursor = ['nw','n','ne','e','se','s','sw','w'];
	      }else if(options.pageAngle>315 && options.pageAngle<=360){
	      	cursor = ['n','ne','e','se','s','sw','w','nw'];
	      }
	      
      	$.getByClass(cropBox, 'line-n')[0].style.cursor = cursor[0]+'-resize';
      	$.getByClass(cropBox, 'line-e')[0].style.cursor = cursor[2]+'-resize';
      	$.getByClass(cropBox, 'line-s')[0].style.cursor = cursor[4]+'-resize';
      	$.getByClass(cropBox, 'line-w')[0].style.cursor = cursor[6]+'-resize';
      	
      	$.getByClass(cropBox, 'point-n')[0].style.cursor = cursor[0]+'-resize';
      	$.getByClass(cropBox, 'point-ne')[0].style.cursor = cursor[1]+'-resize';
      	$.getByClass(cropBox, 'point-e')[0].style.cursor = cursor[2]+'-resize';
      	$.getByClass(cropBox, 'point-se')[0].style.cursor = cursor[3]+'-resize';
      	$.getByClass(cropBox, 'point-s')[0].style.cursor = cursor[4]+'-resize';
      	$.getByClass(cropBox, 'point-sw')[0].style.cursor = cursor[5]+'-resize';
      	$.getByClass(cropBox, 'point-w')[0].style.cursor = cursor[6]+'-resize';
      	$.getByClass(cropBox, 'point-nw')[0].style.cursor = cursor[7]+'-resize';
...
/***/ function(module, exports) {
	...
	exports.transferPageAngle = transferPageAngle;//add:overall rotate
	...
	//add:overall rotate
	function transferPageAngle(pageX0,pageY0,angle){//angle为度数		
		var x,y;
		pageX0 = Math.round(pageX0);
		pageY0 = Math.round(pageY0);
		if(Math.abs(pageX0)!=Math.abs(pageY0)){
			x = Math.cos(Math.atan2(pageY0,pageX0)-angle*Math.PI/180)*Math.sqrt(pageX0*pageX0+pageY0*pageY0);
			y = Math.sin(Math.atan2(pageY0,pageX0)-angle*Math.PI/180)*Math.sqrt(pageX0*pageX0+pageY0*pageY0);
		}else{			
			if(pageX0*pageY0>0){
				x = Math.cos(Math.atan2(pageX0,pageY0)-angle*Math.PI/180)*Math.sqrt(pageX0*pageX0+pageY0*pageY0);
				y = Math.sin(Math.atan2(pageX0,pageY0)-angle*Math.PI/180)*Math.sqrt(pageX0*pageX0+pageY0*pageY0);
			}else{
				x = -Math.cos(Math.atan2(pageX0,pageY0)-angle*Math.PI/180)*Math.sqrt(pageX0*pageX0+pageY0*pageY0);
				y = -Math.sin(Math.atan2(pageX0,pageY0)-angle*Math.PI/180)*Math.sqrt(pageX0*pageX0+pageY0*pageY0);
			}
	  ...
	  change: function change(shiftKey, originalEvent) {
	    ...
	    /*var range = {
	      ...
	      y: self.endY - self.startY	      
			...
			//add:overall rotate
			var rangeX0 = self.endX - self.startX;
			var rangeY0 = self.endY - self.startY;
			var range = {
	      x: $.transferPageAngle(rangeX0,rangeY0,options.pageAngle).x,
	      y: $.transferPageAngle(rangeX0,rangeY0,options.pageAngle).y	      
	    };
	  ...
	  getData: function getData(rounded) {
	    ...
	    if (self.ready && self.cropped) {
	      ...
	      $.each(data, function (n, i) {
	        ...
	        if(i=='x' ||i=='y'){//坐标
	        	if(n<0){
					data[i] = rounded ? Math.floor(n) : n;//精度向下取整
				}else {
					data[i] = rounded ? Math.ceil(n) : n;//精度向上整
				}
			}else{//宽高均向下取整
				data[i] = rounded ? Math.floor(n) : n;//精度问题向下取整
			}
